...
- この書き込み直後のツール確認はLPPSへの書き込み・復帰とStudio基本通信のみ。LPPS自身を起点とするCDC再書き込み試験はまだ行っていない。

## 最新LPPS版の入力・LED所有者確認

上記artifact@b3191a4の書き込み後、「まずLPPSの入力とLED表示を試してください」と案内し、所有者から「OKです」と返答を得た。この今回版への明示的な確認として、入力とLED表示（内蔵RGB/SPI LED併用・USB表示・レイヤー番号点滅）を所有者申告の合格にする。

以前の入力合格の転用ではない。この時点ではLPPS自身からのCDC起動・復帰を保留したが、後続の[2026-09-01 CDC確認](madula-lpps-cdc-recovery-2026-09-01.md)で合格。IQSやStudio内の編集・保存など、依頼した確認範囲を超える合格には広げない。
